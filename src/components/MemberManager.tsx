...
import { Plus, Users, UserPlus, Phone, Mail, ToggleLeft, ToggleRight, Trash2, Shield, CircleAlert } from 'lucide-react';
        ...
        <div className="tech-box p-6 bg-white space-y-4">
                    ...
                    {isManager ? (
                      <>
                        ...
                        <button
                          ...
                          className={`px-2 py-1 text-[10px] font-mono font-bold uppercase border border-[#141414] text-[#141414] hover:bg-[#141414] hover:text-white transition-all cursor-pointer`}
                        >
                        ...
                        {members.length > 2 && (
                          <button
                            onClick={() => {
                              if (confirm(`Remove ${member.name} permanently from system? Previous historical records won't be deleted but will belong to an orphan ID.`)) {
          ...
          <div className="p-3.5 bg-[#F0EFEC] border border-[#141414] flex items-start space-x-2 mt-4">
            ...
            <p className="text-[10px] font-mono uppercase text-[#141414]/80 leading-relaxed font-bold">
              <strong>Status behavior note:</strong> Paused/inactive members temporary skipped and won't split shared utility charges (like Bua/Cook, WIFI, or electricity) for the cycle.
            </p>
          </div>
        </div>
      </div>
